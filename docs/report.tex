...
This project builds a Kubernetes operator that measures pod-to-pod connectivity from multiple nodes. It deploys one lightweight probe agent on each node, asks these agents to probe each other, builds a directed reachability matrix, classifies the shape of failures, exposes the result through a Kubernetes custom resource, and records measurements for evaluation. The project is divided into five implementation areas: platform setup, probe agent, operator/controller, analysis/classification, and evaluation.
...
During the run, F4 printed Kubernetes warnings because the script intentionally uses forced deletion of the Calico pod to create a node/CNI disruption. F5 timed out in all three trials. This means the 40 percent packet-loss condition did not make the live \code{NetworkHealthCheck} transition to an unhealthy state within the 120 second polling window. The F6 cleanup printed \code{NotFound} messages when the deny-all policy was already absent; this is an idempotent cleanup message and did not prevent the F6 trials from running.
...
The F5 timeout is also informative. It shows that the current live unhealthy transition is tuned more strongly for clear connectivity failure than for partial degradation. The agent records loss, jitter, burst loss, and throughput information, but the controller did not convert the 40 percent packet-loss experiment into an unhealthy status within the configured timeout. This should be treated as a limitation and future tuning point rather than a successful detection.
