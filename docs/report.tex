...
\section{P1: Platform and Deployment}
...
\section{P2: Probe Agent}

P2 implements the agent that runs on every node. The agent is deployed as a DaemonSet, which means Kubernetes schedules one copy on each node. The manifest is in \path{config/agent/daemonset.yaml}.
...
\section{P3: Operator and Kubernetes API}

P3 implements the operator logic. The operator periodically reads \code{NetworkHealthCheck} resources, discovers running agent pods, asks agents to probe each other, stores the matrix, and writes status back to Kubernetes.
...
The controller preserves \code{lastTransitionTime} when the condition has not actually changed. This is important for P5 because detection latency depends on the time when the system first moved from healthy to unhealthy.

\section{P4: Analysis and Classification}
...
\section{P5: Evaluation}

P5 evaluates the system using repeatable experiments. The key script is \code{hack/measure-mttd.sh}. It clears previous faults, injects a chosen scenario, records the injection timestamp, polls the \code{NetworkHealthCheck} status, and writes a CSV row when an unhealthy transition is observed.
...
\begin{table}[h]
\centering
\caption{Specific scenarios used in P5}
...
For the main P5 run, F2 through F6 are repeated because they are actual injected faults. F1 is used as the healthy starting point and cleanup baseline before each trial. This keeps each trial independent from the previous one.
...
\begin{table}[h]
\centering
\caption{CSV columns produced by the P5 scripts}
\begin{tabularx}{\textwidth}{@{}lY@{}}
\toprule
...
This report is designed to be completed with measured results after running the evaluation on the cluster. The expected generated summary table has the form shown in Table~\ref{tab:results-template}. It should be replaced or supplemented by \code{docs/results/p5-summary.md} after the experiments are run.

\begin{table}[h]
\centering
\caption{Results table template}
\label{tab:results-template}
\scriptsize
\begin{tabularx}{\textwidth}{@{}lYYYYY@{}}
\toprule
\textbf{Scenario} & \textbf{Trials} & \textbf{Accuracy} & \textbf{Avg MTTD ms} & \textbf{Avg RTT ms} & \textbf{Avg loss rate} \\
\midrule
